...
const userSlice = createSlice({
  ...
  reducers: {
    ...
    acceptTask: (state, { payload }) => {
      const { id } = payload;
      state.authUser.tasks = state.authUser.tasks.map((x) =>
        x.id === id
          ? (x.accepted = true)
    ...
    rejectTask: (state, { payload }) => {
      const { id } = payload;
      state.authUser.tasks = state.authUser.tasks.filter((x) => x.id !== id);
    ...
    assignTasks: (state, { payload }) => {
      ...
      state.msg = status
        ? `Task(s) successfully assigned to user with ID: ${userID}`
        : "";
    },
  },
