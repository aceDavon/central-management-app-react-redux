...
import { addTask, catchErr, clearModal, selectAllTasks } from "./taskSlice";
...
const CreateTask = () => {
  ...
  const handleSubmit = () => {
    ...
    validate()
      ? dispatch(addTask(title, description, category, excerpts, isoDate))
      : catchErr("failed");
  };
  return (
    <form className="xl:ml-20 xl:w-5/12 lg:w-5/12 md:w-8/12 mb-12 md:mb-0 relative">
      {reports.taskAdd !== null && (
        <CommonModal
          modalBody={modalBody}
          modalHeader={modalHeader}
          btnText={btnText}
          dispatcher={clearModal}
        />
      )}
      <div className="mb-6">
        <CommonInput
